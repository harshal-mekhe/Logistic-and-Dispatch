...
router.get("/customers/available", async (req, res) => {
  ...
  try {
    ...
    console.error(err);
    res.status(500).json({ message: "Error loading available customers" });
...
router.get("/next/:id", async (req, res) => {
  ...
  try {
    ...
    const result = await connection.execute(
      `SELECT ORDERID, CUSTOMERID, DISPATCHDATE, SOURCE, DESTINATION, STATUS 
       FROM DISPATCHORDER 
       WHERE ORDERID = (
         SELECT MIN(ORDERID) FROM DISPATCHORDER WHERE ORDERID > :1
       )`,
      [req.params.id]
    );
  ...
  try {
    ...
    const result = await connection.execute(
      `SELECT ORDERID, CUSTOMERID, DISPATCHDATE, SOURCE, DESTINATION, STATUS 
       FROM DISPATCHORDER 
       WHERE ORDERID = (
         SELECT MAX(ORDERID) FROM DISPATCHORDER WHERE ORDERID < :1
       )`,
      [req.params.id]
    );
...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    const dispatchOrders = result.rows.map(row => ({
      orderId: row[0],
      customerId: row[1],
      dispatchDate: row[2],
      source: row[3],
  ...
  try {
    ...
    if (!dispatchDate || dispatchDate === "") {
      await connection.execute(
        `INSERT INTO DISPATCHORDER (CUSTOMERID, SOURCE, DESTINATION, STATUS) 
         VALUES (:1, :2, :3, :4)`,
      ...
      await connection.execute(
        `INSERT INTO DISPATCHORDER (CUSTOMERID, SOURCE, DESTINATION, DISPATCHDATE, STATUS) 
      ...
      await connection.execute(
        `UPDATE DISPATCHORDER 
         SET CUSTOMERID = :1, 
             SOURCE = :2, 
             DESTINATION = :3, 
             DISPATCHDATE = TO_DATE(:4, 'YYYY-MM-DD'), 
             STATUS = :5 
         WHERE ORDERID = :6`,
